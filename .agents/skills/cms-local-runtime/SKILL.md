...
- Tomcat candidate paths: first check `C:\Tomcat 9.0` for `bin\catalina.bat`.
...
- Start Tomcat as a detached Windows process with `Start-Process -FilePath "$TomcatHome\bin\catalina.bat" -ArgumentList "start"`.
- Never start Tomcat by directly invoking `& "$TomcatHome\bin\catalina.bat" start` inside the tool session.
- The `Start-Process` startup command must be the final tool call in the startup flow.
- After the `Start-Process` tool call returns, do not run any other tool call, command, healthcheck, file read, todo/status update, or verification.
